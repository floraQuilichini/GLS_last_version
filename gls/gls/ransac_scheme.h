...
public:
	...
	std::pair<double, int> compute_min(std::vector<std::pair<double, int>>& vec, int index_to_remove = -1);
	...
	Eigen::Matrix4d ransac_algorithm(int nb_iterations, Scalar max_err_scale, Scalar max_err_reg, Scalar max_err_norm, Scalar bbox_diag_ratio);
};