...
void set_point_profiles_cost(std::vector<std::tuple<Point, std::vector<std::tuple<Scalar, Scalar, Scalar>>, std::vector<Scalar>>>& point_gls_profiles, std::map<int, std::tuple<Point, Eigen::ArrayX3d, Scalar>>& map_index_point_profiles_cost, int nb_samples, int alpha = 1.0);