...
VectorType PointMap::get_mean_pos()
{
	 VectorType mean_pos;
	for (auto it = map_index_point_profiles_cost_.begin(); it != map_index_point_profiles_cost_.end(); it++)
		mean_pos += std::get<0>(it->second).pos();
	
	int nb_points = map_index_point_profiles_cost_.size();
	return mean_pos / (Scalar)nb_points;
}
...
std::vector<std::tuple<std::pair<int, int>, Scalar, Scalar, Scalar>> compute_symmetric_pairs(PointMap& map_index_profiles_cost_source, PointMap& map_index_profiles_cost_target, Scalar ratio, int k, VectorType w, Scalar alpha, bool cross_check)
{
	...
	for (auto it_t  = map_index_profiles_cost_target.get_iterator_range().first; it_t != map_index_profiles_cost_target.get_iterator_range().second; it_t++)
	{
		...
		//std::string output_filename = "C:\\Registration\\test_gls_algo\\matching_pairs\\source_matching_pairs_unsorted.txt";
		//std::string output_filename = "C:\\Registration\\RoPS_tests\\heart_clean_full_res_full_heart_rescaled_matching_pairs_unsorted.txt";
		std::string output_filename = "C:\\Registration\\RoPS_tests\\ObjetSynthetique_simp32_heart_synthetique_matching_pairs_unsorted.txt";
