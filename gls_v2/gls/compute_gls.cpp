...
int main(int argc, char** argv)
{
	...
	//Eigen::Matrix3d rescaling_mat = compute_pca_bbox_rescaling(map_index_profiles_cost_source);
	//std::cout << "rescaling mat : " << rescaling_mat << std::endl;
	...
	std::vector<std::tuple<std::pair<int, int>, Scalar, Scalar, Scalar>> symmetric_pairs = compute_symmetric_pairs(map_index_profiles_cost_source, map_index_profiles_cost_target, ratio, 1, w, 4.0, false);
	...
	Scalar bbox_diag = 155.0;//155.0;//25.0;
	Scalar lambda = 0.0;
	for (int i = 0; i < 1; i += 1)
	{
